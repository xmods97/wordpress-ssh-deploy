...
run_deploy() {
	...
	FIXTURE_FAIL_URL_REWRITE="${FIXTURE_FAIL_URL_REWRITE:-0}" \
	FIXTURE_REWRITE_DRY_RUN_REMAINING="${FIXTURE_REWRITE_DRY_RUN_REMAINING:-0}" \
	ENVIRONMENT='staging' \
	...
	DEPLOY_MODE='db' \
	SQL_FILE="$target_root/tmp/local-db-fixture.sql" \
	UPLOADS_ZIP='' \
...
setup_fixture
FIXTURE_FAIL_URL_REWRITE=1
export FIXTURE_FAIL_URL_REWRITE
output="$(run_deploy)"
case "$output" in *'URL rewrite failed; rollback completed'*) ;; *) echo "$output" >&2; exit 1 ;; esac
assert_common_rollback

setup_fixture
unset FIXTURE_FAIL_URL_REWRITE
FIXTURE_REWRITE_DRY_RUN_REMAINING=1
export FIXTURE_REWRITE_DRY_RUN_REMAINING
output="$(run_deploy)"
case "$output" in *'URL rewrite verification failed; rollback completed'*) ;; *) echo "$output" >&2; exit 1 ;; esac
assert_common_rollback

setup_fixture
unset FIXTURE_REWRITE_DRY_RUN_REMAINING
output="$(run_deploy)"
case "$output" in *'WordPress deployment completed (db)'*) ;; *) echo "$output" >&2; exit 1 ;; esac
[ "$(wc -l < "$target_root/mysql-calls.log")" -eq 1 ] || { echo 'Successful rewrite unexpectedly rolled back' >&2; exit 1; }
grep -F -- '--all-tables-with-prefix' "$target_root/php-calls.log" >/dev/null || { echo 'Rewrite did not use the site table prefix' >&2; exit 1; }
grep -F -- '--skip-columns=guid' "$target_root/php-calls.log" >/dev/null || { echo 'Rewrite did not preserve GUID values' >&2; exit 1; }
grep -F -- '--dry-run --format=count' "$target_root/php-calls.log" >/dev/null || { echo 'Rewrite was not verified by dry run' >&2; exit 1; }
find "$target_root/backups" -maxdepth 1 -type f -name 'db-*.sql*' -size +0c | grep -q . || { echo 'Successful rewrite backup was not preserved' >&2; exit 1; }
[ ! -e "$target_root/tmp/local-db-fixture.sql" ] || { echo 'Successful incoming SQL was not cleaned' >&2; exit 1; }
[ ! -d "$target_root/lock/deploy.lock" ] || { echo 'Successful rewrite lock was not cleaned' >&2; exit 1; }
